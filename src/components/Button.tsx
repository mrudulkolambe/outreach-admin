...
const Button = ({ disabled, text, loading, type, className, onClick }: ButtonTypes) => {
  return (
    <button onClick={onClick} className={'flex items-center justify-center bg-accent rounded-xl disabled:cursor-not-allowed disabled:bg-accent/70 px-3 py-3 h-[50px] text-white font-semibold text-sm ' + className} type={type} disabled={disabled || loading}>{loading ? <svg aria-hidden="true" className="w-5 h-5 text-gray-200 animate-spin fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
